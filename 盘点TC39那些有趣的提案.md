...
> 让我们来了解下从娘胎到出生的五个阶段：Strawman（Stage 0，稻草人），Proposal（Stage 1，提议），Draft（Stage 2，草案），Candidate（Stage 3，候选）以及 Finished（Stage 4，完成）。分别对应着：有机会写入标准，讨论实现方法和可能存在的挑战，明确地用正式文档语言描述语法和语义，需要实现和用户的反馈来做细微的改进，准备好进入 ECMAScript 标准。
